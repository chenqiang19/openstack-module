...
public class Users extends AbstractManager<User> implements UserManager {
    private static final String PREFIX = "/v3/users";
    ...
    @Override
    public User changePassword(String id, String oldPwd, String newPwd) throws OperationException {
        JSONObject user = new JSONObject();
        JSONObject property = new JSONObject();
        property.put("password", newPwd);
        property.put("original_password", oldPwd);
        user.put("user", property);
        return _post(PREFIX + "/" + id + "/password", user);
    }
