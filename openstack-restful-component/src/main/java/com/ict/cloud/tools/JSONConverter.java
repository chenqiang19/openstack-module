...
public class JSONConverter {
    /* JSONConverter log handle*/
    private static final Logger logger = LogManager.getFormatterLogger(JSONConverter.class);
    /**
    ...
    public static <T extends AbstractEntity> T jSONToEntity(JSONObject header,JSONObject body, Class<T> cl) {
        return (T)Reflector.newEntity(cl, header, body);
    }
    /**
    ...
    public static <T extends AbstractEntity> T responseToEntity(String body, String what, Class<T> objClass, String region) {
        ...
        if (what.equals("")) {
            e = jSONToEntity(bodyObject, objClass);
        } else if(what.equals("node")){
            e = jSONToEntity(bodyObject, objClass);
        }else {
            if(bodyObject.has(what)){
                e = jSONToEntity(bodyObject.getJSONObject(what), objClass);
            }else{
                e = jSONToEntity(bodyObject, objClass);
            }
        }
    ...
    public static <T extends AbstractEntity> T responseToEntity(String header, String body, String what, Class<T> objClass, String region) {
        //System.out.println(body);
        /* 如果what未设置，使用注解设置的值 */
        //logger.info("responseToEntity start ");
        if (what == null) {
            what = AnnotationProcessor.getEntityName(objClass);
        }
        //logger.info("what is:" + what);
        T e = null;
        /** 如果名称为""，则表示没有名称，比如{"name":"Mary", "age": 20} */
        if (what.equals("v3")) {
            e = jSONToEntity(new JSONObject(header), new JSONObject(body), objClass);
        } else {
            e = jSONToEntity(new JSONObject(header), new JSONObject(body).getJSONObject(what), objClass);
        }
        /* region需要单独设置 */
        e.setRegion(region);
    ...
    public static <T extends AbstractEntity> List<T> responseToEntityList(String body, String what, Class<T> objClass, String region) {
        ...
        JSONObject objs = new JSONObject(body);
        JSONArray entityList = null;

        if (objs.has(what)) {
            entityList = new JSONObject(body).getJSONArray(what);
        } else {
            what = what + "s";
            if (objs.has(what)) {
                entityList = new JSONObject(body).getJSONArray(what);
        ...
        List<T> entities = JSONArrayToEntityList(entityList, objClass);
