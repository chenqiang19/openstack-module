...
public abstract class AbstractManager<T extends AbstractEntity> extends Manager<T> {
    ...
    public String getEndpoint() {
        String endpoint = credentical.getEndpoint(CatalogType.image);
        if(endpoint.contains("controller")){
            return endpoint.replace("controller",openStackConfig.getIp());
        }else {
            return endpoint.replace("10.101.1.253", openStackConfig.getIp());
        }
    }
}
