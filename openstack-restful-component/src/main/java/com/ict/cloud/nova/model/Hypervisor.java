...
public class Hypervisor extends AbstractEntity {
	...
	public Hypervisor(JSONObject hypervisor) {
		...
		if(hypervisor.has("vcpus_used")) {
			this.vcpusUsed = hypervisor.optInt("vcpus_used");
		}
		if(hypervisor.has("vcpus")) {
			this.vcpus = hypervisor.optInt("vcpus");
		}
		if(hypervisor.has("local_gb_used")) {
			this.diskUsed = hypervisor.optInt("local_gb_used");
		}
		if(hypervisor.has("local_gb")) {
			this.disk = hypervisor.optInt("local_gb");
		}
		if(hypervisor.has("free_disk_gb")) {
			this.freeDisk = hypervisor.optInt("free_disk_gb");
		}
		if(hypervisor.has("host_ip")) {
			this.hostIp = hypervisor.optString("host_ip");
		}

		if(hypervisor.has("memory_mb_used")) {
			this.ramUsed = hypervisor.optInt("memory_mb_used");
		}
		if(hypervisor.has("memory_mb")) {
			this.ram = hypervisor.optInt("memory_mb");
		}
		if(hypervisor.has("free_ram_mb")) {
			this.freeRam = hypervisor.optInt("free_ram_mb");
		}
		if(hypervisor.has("hypervisor_type")) {
			this.type = hypervisor.optString("hypervisor_type");
		}

		if(hypervisor.has("hypervisor_version")) {
			this.version = hypervisor.optLong("hypervisor_version");
		}
	}
